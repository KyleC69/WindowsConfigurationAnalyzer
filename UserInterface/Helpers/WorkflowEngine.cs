...
public class WorkflowEngine
{
    ...
    {
        var resultSet = new WorkflowResultSet
        {
        ...
        foreach (var rule in workflow.Rules)
        {
            ...
            var ruleResult = await  ExecuteRuleAsync(rule, workflow.GlobalParameters);
    ...
    private async Task<RuleResult> ExecuteRuleAsync(RuleContract rule, WorkflowParameters globalParams)
    {
        ...
        try
        {
            ...
            var probeData = ResolveProbe(rule.Probe);
            ...
            // Gate 5: Audit trail
            if (rule.EnableAuditTrail)
                LogAudit(rule, result);
        }
        catch (Exception ex)
        {
    ...
    private void ValidateParameters(List<RuleParameter> parameters, WorkflowParameters globalParams)
    {
        try
        {
            ...
            {
                var value = globalParams.GetType().GetProperty(param.Name)?.GetValue(globalParams);
                if (value == null && param.DefaultValue == null)
                    throw new InvalidOperationException($"Missing parameter: {param.Name}");
            }
        }
        catch (Exception e)
        {
    ...
    private void ExecuteActions(List<RuleAction> actions)
    {
        ...
        foreach (var action in actions)
        {
            ...
            var actionTypeObj = action.GetType().GetProperty("ActionType")?.GetValue(action);
            var targetObj = action.GetType().GetProperty("Target")?.GetValue(action);
            var valueObj = action.GetType().GetProperty("Value")?.GetValue(action);

            var actionType = actionTypeObj?.ToString();
            ...
            switch (actionType)
            {
                case "SetEnvironmentVariable":
                    ...
                    if (!string.IsNullOrWhiteSpace(target) && valueObj is string envValue)
                        Environment.SetEnvironmentVariable(target, envValue);
                    break;
                ...
                case "WriteFile":
                    ...
                    {
                        var path = target.StartsWith("file:", StringComparison.OrdinalIgnoreCase) ? target[5..] : target;
                        System.IO.File.WriteAllText(path, writeContent);
                    }
                    break;
                ...
                case "AppendFile":
                    ...
                    {
                        var path = target.StartsWith("file:", StringComparison.OrdinalIgnoreCase) ? target[5..] : target;
                        System.IO.File.AppendAllText(path, appendContent);
                    }
                    break;
                ...
                case "SetRegistryValue":
                    ...
                    if (!string.IsNullOrWhiteSpace(target))
                    {
                        ...
                        var spec = target.StartsWith(prefix, StringComparison.OrdinalIgnoreCase) ? target[prefix.Length..] : target;
                        var parts = spec.Split('|', 2);
                        if (parts.Length == 2)
                        {
                            var fullKeyPath = parts[0];
                            ...
                            if (firstSlash > 0)
                            {
                                var hiveName = fullKeyPath[..firstSlash];
                                var subKey = fullKeyPath[(firstSlash + 1)..];

                                Microsoft.Win32.RegistryKey baseKey = hiveName switch
                                {
                ...
                default:
                    ...
                    var exec = action.GetType().GetMethod("Execute", System.Reflection.BindingFlags.Instance | System.Reflection.BindingFlags.Public | System.Reflection.BindingFlags.NonPublic);
                    if (exec != null && exec.GetParameters().Length == 0)
                        exec.Invoke(action, null);
                    break;
            }
        }
    }
