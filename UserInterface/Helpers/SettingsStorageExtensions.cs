...
public static class SettingsStorageExtensions
{
    ...
    /// This method checks the availability of roaming storage by verifying the <see cref="ApplicationData.RoamingStorageQuota"/> value.
    /// </remarks>
    ...
    /// <summary>
    /// Reads and deserializes a JSON file from the specified <see cref="StorageFolder"/> into an object of type <typeparamref name="T"/>.
    /// </summary>
    ...
    /// deserializes it from JSON format, and returns the resulting object. If the file does not exist, it returns the default value of <typeparamref name="T"/>.
    /// </remarks>
    /// <exception cref="InvalidOperationException">Thrown if the JSON content cannot be deserialized into the specified type <typeparamref name="T"/>.</exception>
    /// <example>
    ...
    public static async Task SaveAsync<T>(this ApplicationDataContainer settings, string key, T value)
    {
        ...
        var str = await Json.StringifyAsync(value);
    ...
    public static void SaveString(this ApplicationDataContainer settings, string key, string value)
    {
        ...
        using var appKey = GetAppRegistryKey(writable: true);
        appKey?.SetValue(key, value, Microsoft.Win32.RegistryValueKind.String);
    }
    ...
    /// <returns>
    /// A task that represents the asynchronous operation. The task result contains the value of type <typeparamref name="T"/> 
    ...
    public static async Task<T?> ReadAsync<T>(this ApplicationDataContainer settings, string key)
    {
        ...
        using var appKey = GetAppRegistryKey(writable: false);
    ...
    /// <param name="options">
    /// Specifies the <see cref="CreationCollisionOption"/> to determine the behavior if a file with the same name already exists.
                                                            ...
                                                            CreationCollisionOption.ReplaceExisting)
    {
        ...
        {
            throw new ArgumentException("Log filename is null or empty. Specify a valid file name.");
        }
        ...
        if (fileExists && options == CreationCollisionOption.FailIfExists)
        {
            throw new IOException($"File already exists: {fullPath}");
        }

        if (!fileExists || options == CreationCollisionOption.ReplaceExisting)
        {
    ...
    /// <returns>
    /// A <see cref="Task"/> representing the asynchronous operation. The result contains the file content as a byte array,
    /// or <c>null</c> if the file does not exist or is not of type <see cref="StorageItemTypes.File"/>.
    /// </returns>
    /// <remarks>
    /// This method attempts to locate the specified file in the provided folder. If the file exists and is of the correct type,
    /// its content is read and returned as a byte array. If the file does not exist or is not a file, the method returns <c>null</c>.
    /// </remarks>
    /// <exception cref="ArgumentNullException">Thrown when <paramref name="folder"/> or <paramref name="fileName"/> is <c>null</c>.</exception>
    ...
    private static Microsoft.Win32.RegistryKey? GetAppRegistryKey(bool writable)
    {
        var subKey = $"Software\\{RegistryCompany}\\{RegistryProduct}";
        return writable
            ? Microsoft.Win32.Registry.CurrentUser.CreateSubKey(subKey, true)
            : Microsoft.Win32.Registry.CurrentUser.OpenSubKey(subKey, false);
    }
}