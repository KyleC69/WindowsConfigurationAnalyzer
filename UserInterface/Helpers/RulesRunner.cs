...
public class RulesRunner
{
    ...
    public async Task RunRulesAsync()
    {
        ...
        try
        {
            var input = GatherCurrentUserProfile();
    ...
    private UserProfileInput GatherCurrentUserProfile()
    {
        ...
        try
        {
            ...
            if (!string.IsNullOrWhiteSpace(sid))
            {
                ...
                if (key != null)
                {
                    input.RegistryProfilePath = key.GetValue("ProfileImagePath") as string ?? string.Empty;
                }
            }
        }
        catch
        ...
        try
        {
            ...
            {
                var dirInfo = new DirectoryInfo(input.ProfilePath);
                var security = dirInfo.GetAccessControl();
                using var identity = WindowsIdentity.GetCurrent();
                var principalSids = new HashSet<string>(identity.Groups?.Select(g => g.Value) ?? Enumerable.Empty<string>());
                if (identity.User?.Value is string userSid)
                    principalSids.Add(userSid);
                    ...
                    if (rule.AccessControlType == AccessControlType.Allow && principalSids.Contains(sid))
                    {
                        foreach (FileSystemRights val in Enum.GetValues<FileSystemRights>())
                        {
                            if (val != 0 && rule.FileSystemRights.HasFlag(val))
                            {
                                ...
                                if (!input.AclRights.Contains(name))
                                    input.AclRights.Add(name);
                            }
                        }
                    }
                }
        ...
        try
        {
            using var sc = new ServiceController("ProfSvc");
        ...
        try
        {
            ...
            for (var i = count - 1; i >= 0 && i > count - 1000; i--)
            {
                ...
                if (entry.Source == "User Profile Service" && (entry.InstanceId == 1509 || entry.InstanceId == 1511))
                {
                    ids.Add((int)entry.InstanceId);
                }
            }
...
public class UserProfileInput
{
    ...
    public List<string> StandardFolders { get; set; } = new();
    public List<string> AclRights { get; set; } = new();
