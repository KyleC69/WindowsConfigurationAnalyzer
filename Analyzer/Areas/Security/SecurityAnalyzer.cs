...
public sealed class SecurityAnalyzer : IAnalyzerModule
{
	...
	public Task<AreaResult> AnalyzeAsync(IAnalyzerContext context, CancellationToken cancellationToken)
	{
		...
		// Sections
		var secCenter = new Dictionary<string, object?>();
		var defender = new Dictionary<string, object?>();
		var deviceGuard = new Dictionary<string, object?>();
		var secureBoot = new Dictionary<string, object?>();
		var bitlocker = new List<object>();
		var rdp = new Dictionary<string, object?>();
		var smb = new Dictionary<string, object?>();
		var lsa = new Dictionary<string, object?>();
		var windowsUpdate = new Dictionary<string, object?>();
		...
		try
		{
			...
			var av = new List<object>();
			var fw = new List<object>();
			var asw = new List<object>();
			try
			{
			...
			try
			{
				foreach (var mo in context.Cim.Query(
					...
					fw.Add(new
					{
						Name = mo.GetOrDefault("displayName"), Path = mo.GetOrDefault("pathToSignedProductExe"),
						State = mo.GetOrDefault("productState"), Timestamp = mo.GetOrDefault("timestamp")
					});
			}
			catch (Exception ex)
			{
		...
		try
		{
			...
			var v = context.Registry.GetValue("HKLM\\SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Policies\\System",
				"EnableLUA");
		...
		try
		{
			...
			foreach (var s in context.Cim.Query(
				...
				services.Add(new
				{
					...
					StartMode = s.GetOrDefault("StartMode"), PathName = s.GetOrDefault("PathName"),
		...
		try
		{
			...
			foreach (var sb in context.Cim.Query("SELECT SecureBootEnabled FROM MS_SecureBoot", "\\\\.\\root\\wmi"))
			{
		...
		try
		{
				         ...
				         "\\\\.\\root\\CIMV2\\Security\\MicrosoftVolumeEncryption"))
			{
				...
				if (ps is uint u && u == 1) bitlockerProtected++;

				bitlocker.Add(new
				{
					DeviceID = vol.GetOrDefault("DeviceID"), ProtectionStatus = ps,
