...
public sealed class HardwareAnalyzer : IAnalyzerModule
{
	...
	public Task<AreaResult> AnalyzeAsync(IAnalyzerContext context, CancellationToken cancellationToken)
	{
		...
		var memoryModules = new List<object>();
		var disks = new List<object>();
		var partitions = new List<object>();
		var volumes = new List<object>();
		var gpu = new List<object>();
		var baseboard = new Dictionary<string, object?>();
		var enclosure = new Dictionary<string, object?>();
		var tpm = new Dictionary<string, object?>();
		var battery = new List<object>();
		double totalMemGb = 0;
		...
		var summary = new
		{
			CPUCount = cpuList.Count, MemoryGB = totalMemGb, DiskCount = disks.Count, GPUCount = gpu.Count,
