...
using WindowsConfigurationAnalyzer.Export;
using WindowsConfigurationAnalyzer.Diagnostics;
...
public static class DemoRun
{
 ...
 var engine = new WindowsConfigurationAnalyzer.Engine.AnalyzerEngine(logger);
 var ctx = services.GetRequiredService<WindowsConfigurationAnalyzer.Contracts.IAnalyzerContext>();
 foreach (var m in services.GetServices<WindowsConfigurationAnalyzer.Contracts.IAnalyzerModule>())
 engine.AddModule(m);
 var result = await engine.RunAllAsync(ctx, ct);

 // Evaluate rules (temporary, few starters)
 var ruleEngine = new RuleEngine(new WindowsConfigurationAnalyzer.Contracts.IRule[]
 {
 new AvMissingRule(),
