...
public static class WCADiagnosticsProvider
{
	...
	{
		if (!m_provider.IsEnabled()) return true;
...
internal sealed class WCADiagnosticsEventSource : EventSource
{
	...
	{
		if (IsEnabled()) WriteEvent(0x3e9, SessionId, Computer, Version, CorrelationId);
	}
	...
	{
		if (IsEnabled()) WriteEvent(0x7d1, new object[] { Module, CorrelationId, ItemCount, DurationMs, Details });
	}
	...
	public void HardwareAnalysis(string Module, string CorrelationId, uint FindingsCount, string Severity,
		string Summary)
	{
		if (IsEnabled()) WriteEvent(0x7d2, new object[] { Module, CorrelationId, FindingsCount, Severity, Summary });
	}
	...
	{
		if (IsEnabled()) WriteEvent(0x899, new object[] { Module, CorrelationId, ItemCount, DurationMs, Details });
	}
	...
	{
		if (IsEnabled()) WriteEvent(0x89a, new object[] { Module, CorrelationId, FindingsCount, Severity, Summary });
	}
	...
	{
		if (IsEnabled()) WriteEvent(0x961, new object[] { Module, CorrelationId, ItemCount, DurationMs, Details });
	}
	...
	public void SoftwareAnalysis(string Module, string CorrelationId, uint FindingsCount, string Severity,
		string Summary)
	{
		if (IsEnabled()) WriteEvent(0x962, new object[] { Module, CorrelationId, FindingsCount, Severity, Summary });
	}
	...
	{
		if (IsEnabled()) WriteEvent(0xA29, new object[] { Module, CorrelationId, ItemCount, DurationMs, Details });
	}
	...
	public void NetworkAnalysis(string Module, string CorrelationId, uint FindingsCount, string Severity,
		string Summary)
	{
		if (IsEnabled()) WriteEvent(0xA2A, new object[] { Module, CorrelationId, FindingsCount, Severity, Summary });
	}
	...
	{
		if (IsEnabled()) WriteEvent(0xAF1, new object[] { Module, CorrelationId, ItemCount, DurationMs, Details });
	}
	...
	public void SecurityAnalysis(string Module, string CorrelationId, uint FindingsCount, string Severity,
		string Summary)
	{
		if (IsEnabled()) WriteEvent(0xAF2, new object[] { Module, CorrelationId, FindingsCount, Severity, Summary });
	}
	...
	{
		if (IsEnabled()) WriteEvent(0xBB9, new object[] { Module, CorrelationId, ItemCount, DurationMs, Details });
	}
	...
	public void PerformanceAnalysis(string Module, string CorrelationId, uint FindingsCount, string Severity,
		string Summary)
	{
		if (IsEnabled()) WriteEvent(0xBBA, new object[] { Module, CorrelationId, FindingsCount, Severity, Summary });
	}
	...
	{
		if (IsEnabled()) WriteEvent(0xC81, new object[] { Module, CorrelationId, ItemCount, DurationMs, Details });
	}
	...
	public void StartupAnalysis(string Module, string CorrelationId, uint FindingsCount, string Severity,
		string Summary)
	{
		if (IsEnabled()) WriteEvent(0xC82, new object[] { Module, CorrelationId, FindingsCount, Severity, Summary });
	}
	...
	{
		if (IsEnabled()) WriteEvent(0xD49, new object[] { Module, CorrelationId, ItemCount, DurationMs, Details });
	}
	...
	{
		if (IsEnabled()) WriteEvent(0xD4A, new object[] { Module, CorrelationId, FindingsCount, Severity, Summary });
	}
	...
	{
		if (IsEnabled()) WriteEvent(0xE11, new object[] { Module, CorrelationId, ItemCount, DurationMs, Details });
	}
	...
	{
		if (IsEnabled()) WriteEvent(0xE12, new object[] { Module, CorrelationId, FindingsCount, Severity, Summary });
	}
	...
	{
		if (IsEnabled()) WriteEvent(0xE1A, SessionId, Format, Path, CorrelationId);
	}
	...
	{
		if (IsEnabled()) WriteEvent(0xED9, new object[] { Module, CorrelationId, ItemCount, DurationMs, Details });
	}
	...
	public void ReadersAnalysis(string Module, string CorrelationId, uint FindingsCount, string Severity,
		string Summary)
	{
		if (IsEnabled()) WriteEvent(0xEDA, new object[] { Module, CorrelationId, FindingsCount, Severity, Summary });
	}
